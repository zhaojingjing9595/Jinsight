...
type TxListMergedItem = { kind: "transaction"; transaction: Transaction };
...
function mergeTransactionsAndPaidBills(
  ...
  return txRows.sort((a, b) => {
    const dateA = a.transaction.date.getTime();
    const dateB = b.transaction.date.getTime();
    if (dateB !== dateA) return dateB - dateA;
    const tieA = a.transaction.createdAt.getTime();
    const tieB = b.transaction.createdAt.getTime();
    return tieB - tieA;
  });
}
...
function mergedListCategories(items: TxListMergedItem[]): Transaction["category"][] {
  ...
  for (const it of items) {
    uniq.add(it.transaction.category);
  }
...
function mergedItemCategory(it: TxListMergedItem): Transaction["category"] {
  return it.transaction.category;
}
...
function TxListMergedRowTransaction({
  ...
  const isBillPayment = Boolean(t.billId);

  return (
    <div className={`flex flex-col gap-1.5 md:gap-2 border-2 rounded-[10px] md:rounded-[12px] px-2.5 md:px-3 py-1.5 md:py-2 ${isBillPayment ? "border-dashed border-primary/40 bg-primary/[0.04]" : "border-dashed border-ink/60 bg-base"}`}>
...
export function TransactionsCard({
  ...
  async function handleDelete(transaction: Transaction) {
    const message = transaction.billId
      ? "Deleting this transaction will mark the bill as unpaid. Continue?"
